...
export const ScrollingCarousel: FunctionComponent<SliderProps> = ({
	...
	const slide = (direction: SlideDirection) => {
		...
		smoothHorizontalScroll(300, slideAmount)
	};

	const smoothHorizontalScroll = (totalTime: number, scrollAmount: number) => {
		const intervalTime = 20
		const scrollLeftRate = intervalTime / totalTime * scrollAmount
		let totalScrollAmount = 0
		var slideTimer = setInterval(function(){
			if (totalScrollAmount > Math.abs(scrollAmount)){
				window.clearInterval(slideTimer);
			}

			slider.current!.scrollLeft += scrollLeftRate;
			totalScrollAmount += Math.abs(scrollLeftRate);
	...
	const getLeftArrow = (data: string, element?: ReactElement) => {
		return (
			<div className={'Button-Left'} data-arrow={data} onClick={() => slide(SlideDirection.Right)}>
				{element ?? <button />}
			</div>
		)
	};

	const getRightArrow = (data: string, element?: ReactElement) => {
		return (
			<div className={'Button-Right'} data-arrow={data} onClick={() => slide(SlideDirection.Left)}>
				{element ?? <button />}
			</div>
		)
	};
	...
	return (
		<div className={`${styles.sliderBase} ${className}`} data-testid="carousel" style={{position: 'relative'}}>
			{showArrow.left && (
				getLeftArrow( "left", leftIcon)
			)}
			<div
				id={id}
				ref={ref}
